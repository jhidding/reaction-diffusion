# ~\~ language=Python filename=Snakefile
# ~\~ begin <<README.md|Snakefile>>[0]
import numpy as np
import h5py as h5
from numba import njit

# ~\~ begin <<README.md|laplacian>>[0]
@njit
def laplacian(x: np.array):
    ...
    return y
# ~\~ end
# ~\~ begin <<README.md|gray-scott-model>>[0]
def gray_scott_model(F, k, D_u=2e-5, D_v=1e-5, res=0.01):
    ...
    return df
# ~\~ end
# ~\~ begin <<README.md|euler-method>>[0]
...
# ~\~ end
# ~\~ begin <<README.md|initial-state>>[0]
